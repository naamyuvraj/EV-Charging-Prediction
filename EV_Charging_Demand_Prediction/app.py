...
if uploaded_file:
    ...
    if st.button("Predict Demand", type="primary"):
        ...
        ax.set_ylabel("Charging Volume (kWh)")
        ax.set_title("Actual vs Predicted EV Demand")
        ax.legend()
        ...
        ax3.set_title("Monthly Average EV Charging Demand")
        plt.xticks(rotation=45)
        plt.tight_layout()

        fig3.savefig("monthly_average.png")
        st.pyplot(fig3)

        st.success("Images saved successfully!")
        st.write("Saved in directory:", os.getcwd())